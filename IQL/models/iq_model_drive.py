...
class Critic(nn.Module):
    def __init__(self, device, obs_size=7, action_dim=9, seq_len=256, h_dim=256, is_traj=False, num_pref=6):
        ...
        self.embed_o = nn.Embedding(SPEC.spatial_vocab_size, h_dim)
        self.embed_d = nn.Embedding(SPEC.spatial_vocab_size, h_dim)
...
class SoftQ(object):
    ...
    def iq_update_critic(self, expert_batch, od_expert_batch=None):
        ...
        # add entropy term
        entropy = Categorical(logits=q + q_).entropy().mean() if q_ is not None else Categorical(logits=q).entropy().mean()
